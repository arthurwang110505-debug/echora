...
export default function YouTubeCallback() {
  ...
  useEffect(() => {
    ...
    void restoreYouTubeSession().then(() => {
      ...
      window.setTimeout(() => navigate(connected ? '/?youtube=connected' : '/?youtube=error', { replace: true }), connected ? 500 : 1400);
    });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#07090e] px-6 text-center text-white">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-400">{connectionFailed ? '無法驗證授權或讀取帳號資料，將返回首頁顯示修正方式。' : '授權完成後會驗證帳號與歌單存取權。'}</p>
