...
const VisualizerSonnet: React.FC<VisualizerSharedProps> = (props) => {
  ...
  // Latest structural inputs are pushed into the running runtime (which never
  // tears down for tuning/theme changes); the refs also feed runtime creation.
  const tuningRef = useRef<SonnetTuning>(effectiveSonnetTuning);
  tuningRef.current = effectiveSonnetTuning;
  const themeRef = useRef<Theme>(theme);
  themeRef.current = theme;
  const fontScaleRef = useRef(lyricsFontScale);
  fontScaleRef.current = lyricsFontScale;
    ...
    void import("./createSonnetPixiRuntime")
      .then(({ SonnetPixiRuntime }) => {
        ...
        return SonnetPixiRuntime.create({
          ...
          audioPower,
          audioBands,
          lyricsFontScale: fontScaleRef.current,
  ...
  }, [currentTime, program, audioPower, audioBands]);

  // Hot-swap tuning into the running runtime. Per-frame knobs apply immediately;
  // structural changes (layer toggles, MG density, post-process) trigger an
  // incremental background rebuild instead of a teardown, so dragging settings
  // sliders never flashes/destroys the animation.
  useEffect(() => {
    runtimeRef.current?.setTuning(effectiveSonnetTuning);
  }, [effectiveSonnetTuning]);

  // Theme / font scale / static mode change glyph styles and MG artwork. They are
  // pushed as visual-input patches and rebuilt scene-by-scene in the background.
  useEffect(() => {
    runtimeRef.current?.setVisualInputs({
      theme,
      lyricsFontScale,
      staticMode,
    });
  }, [theme, lyricsFontScale, staticMode]);
  ...
  const creditsRecentCompletedLine =
    recentCompletedLine === finalLine ? null : recentCompletedLine;
