...
const CarouselItem: React.FC<CarouselItemProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`rounded-3xl overflow-hidden shadow-2xl relative transition-all duration-300 group ${
          isActive ? 'ring-2 ring-[#62f5c4]/60 shadow-[0_20px_50px_rgba(98,245,196,0.25)]' : 'hover:ring-1 hover:ring-white/30'
        }`}
...
export const Carousel3D: React.FC<Carousel3DProps> = ({
  ...
  const coverSize = isMobile ? 210 : isTablet ? 260 : 310;
  const spacing = isMobile ? 120 : isTablet ? 170 : 210;
    ...
    >
      ...
      >
        {items.map((item, index) => {
          ...
          const opacity = Math.max(0.12, 1 - Math.abs(distance) * 0.28);
      ...
      <AnimatePresence mode="wait">
        {currentSong && (
          <motion.div
            ...
            className="text-center mt-3 z-20 max-w-md px-4"
          >
            <h3 className="text-xl sm:text-2xl font-extrabold font-heading text-white tracking-tight truncate drop-shadow-md">
              {currentSong.title}
            </h3>
