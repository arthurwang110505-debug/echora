...
- [ ] 重現並修正實際 YouTube 暫停按鈕導致的錯誤畫面，不以錯誤邊界掩蓋根因。
- [ ] 將歌詞同步「−／同步／＋」控制放入手機、雙欄與沉浸舞台均可見的操作位置。
- [ ] 歌單選擇側欄展開時暫時隱藏 YouTube 原生播放器，避免遮擋歌單，關閉側欄後再恢復。
- [ ] 逐項重審完整改進企劃，標示已驗收、待補強與受外部憑證限制的項目，完成可實作範圍的回歸測試與正式驗收。
- [ ] 修正切換沉浸舞台時的 DOM `insertBefore` 錯誤，避免全螢幕操作再度進入錯誤頁面。
